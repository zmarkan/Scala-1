...
class ListsSuite extends FunSuite {
  ...
  test("sum empty list returns 0"){
    assert(sum(List()) === 0)
  }

  test("sum 1 element in list returns this element"){
    assert(sum(List(1)) === 1)
    assert(sum(List(10)) === 10)
    assert(sum(List(12345)) === 12345)
  }

  test("max empty list throws NoSuchElementException"){
    intercept[NoSuchElementException]{
      max(List())
    }
  }

  test("max 1 element in list returns this element"){
    assert(max(List(3)) === 3)
  }

  test("max 2 elements in list returns larger of 2"){
    assert(max(List(1, 5)) === 5)
    assert(max(List(10, 3)) === 10)
  }
